...
{
    private static async Task<Result<PackageMetadata>> CollectPackageDataAsync(ProgressContext ctx, CancellationToken ct, string? path = null)
    {
    ...
    public static async Task<Result<(SortResult, string)>> GetModulesToCompileAsync(ProgressContext ctx, CancellationToken ct,
        string? path = null)
    {
        ...
        if (modGraphRes.Ok && !modGraphRes.Value!.HasCycle) return Result<(SortResult, string)>.Pass((modGraphRes.Value, modules.Value.Item2), modGraphRes.Diagnostics);
        ...
        CancellationToken ct, string? path = null)
    {
        ...
        return foundError ? Result<(List<ModuleMetadata>, string)>.Fail(packageDataResult.Diagnostics) : Result<(List<ModuleMetadata>, string)>.Pass((moduleData, packageMetadata.Name), packageDataResult.Diagnostics);
    }
}